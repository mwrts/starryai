...
const ChatPage = ({ characterId }) => {
  ...
  useEffect(() => {
    ...
    if (foundCharacter) {
      ...
      if (history.length > 0) {
        ...
        const firstMessage = foundCharacter.firstMessage || `You are now chatting with ${foundCharacter.name}.`;
  ...
  const handleSkipTurn = async () => {
    ...
    setIsTyping(true);
    try {
      // Call the API with the current messages, prompting the bot to send another message.
